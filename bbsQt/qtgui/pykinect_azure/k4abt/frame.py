...
from ._k4abtTypes import k4abt_body_t, body_colors
from ..k4a import Image, Capture, Transformation
from ..k4a._k4atypes import K4A_CALIBRATION_TYPE_DEPTH
...
class Frame:
	...
	def ex_joints(self, destination_image, dest_camera = K4A_CALIBRATION_TYPE_DEPTH, only_segments = False):
		num_bodies = self.get_num_bodies()
		joint_list = []
		
		for body_id in range(num_bodies):
			joints = self.get_joints(destination_image, body_id, dest_camera, only_segments)
			joint_list.append(joints)

		return joint_list

	def get_joints(self, destination_image, bodyIdx = 0, dest_camera = K4A_CALIBRATION_TYPE_DEPTH, only_segments = False):
		return self.get_body2d(bodyIdx, dest_camera).joint(destination_image, only_segments)
